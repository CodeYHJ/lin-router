...
def test_batch_api_and_manual_priority_source_contract():
    ...
    assert "onAddAggregateMembers" in actions_js
    assert "group_id: groupId" in actions_js
    assert "model_ids: modelIds" in actions_js
    assert "member-add-select-all" in actions_js
    ...
    # 顺序已表达实际调度排位，不能再额外展示内容相同的“优先级”列。
    assert "<th>优先级</th>" not in config_js
    assert 'class="aggregate-member-action-buttons"' in config_js
    action_cell_css = config_css.split(".aggregate-member-actions {", 1)[1].split("}", 1)[0]
    assert "display: table-cell" in action_cell_css
    assert "display: flex" not in action_cell_css
    assert ".aggregate-member-action-buttons > button" in config_css
    assert "height: 28px" in config_css
    assert "min-height: 28px" in config_css
    assert ".aggregate-member-action-buttons > .btn-icon" in config_css
...
def test_combined_add_ui_uses_one_request_one_refresh_and_selected_model_ids():
    script = r'''
const fs = require('fs');
const vm = require('vm');
...
let reloads = 0;
const listeners = {};
const selectedInput = { dataset: { memberAddModelId: 'm-new' }, disabled: false, checked: true };
const groupSelect = { value: 'g1', addEventListener(type, fn) { listeners[`group-${type}`] = fn; } };
const queryInput = { value: '', addEventListener(type, fn) { listeners[`query-${type}`] = fn; } };
const selectAll = { checked: false, indeterminate: false, disabled: false, addEventListener(type, fn) { listeners[`all-${type}`] = fn; } };
const list = {
  innerHTML: '',
  addEventListener(type, fn) { listeners[`list-${type}`] = fn; },
  querySelectorAll(selector) { return selector.includes('data-member-add-model-id') ? [selectedInput] : []; },
};
const summary = { textContent: '' };
const confirmButton = { textContent: '', disabled: false };
const overlay = {
  querySelector(selector) {
    return {
      '#member-add-group': groupSelect,
      '#member-add-query': queryInput,
      '#member-add-select-all': selectAll,
      '#member-add-model-list': list,
      '#member-add-summary': summary,
      '[data-action="confirm"]': confirmButton,
    }[selector] || null;
  },
};
const models = {
  'm-existing': { id: 'm-existing', name: '已有模型', ep_id: 'up-existing', upstream_model: 'up-existing', group_id: 'g1', usable: true },
  'm-new': { id: 'm-new', name: '新增模型', ep_id: 'up-new', upstream_model: 'up-new', group_id: 'g1', usable: true },
  'm-disabled': { id: 'm-disabled', name: '不可用模型', ep_id: 'up-disabled', upstream_model: 'up-disabled', group_id: 'g1', usable: false },
};
const Store = {
  state: { groups: [{ id: 'g1', name: '中转一组', provider_type: 'relay' }] },
  getAggregateMembers() { return [{ id: 'am-existing', aggregate_id: 'a1', group_id: 'g1', model_id: 'm-existing' }]; },
  getModelsByGroup() { return Object.values(models); },
...
const Modal = {
  async form(options) {
    ...
    const error = options.validate({ 'member-add-group': 'g1' });
    if (error) throw new Error(error);
    return { 'member-add-group': 'g1' };
  },
};
const API = {
  async createAggregateMembersBatch(aggregateId, payload) {
    ...
    if (mode === 'duplicate') return { message: '所选模型均已存在', added_count: 0, skipped_count: 1, failed_count: 0 };
    return { added_count: 1, skipped_count: 0, failed_count: 0 };
  },
};
const Toast = {
...
const context = { console, Store, document, Modal, API, Toast, Utils, Array, Number, Set, Map, String, Object };
vm.runInNewContext(source, context);
const controller = { async reloadAfterAggregateMemberChange() { reloads += 1; } };
(async () => {
  await context.actions.onAddAggregateMembers(controller);
  if (requests.length !== 1 || reloads !== 1) throw new Error('success must request and refresh once');
  if (requests[0].aggregateId !== 'a1' || requests[0].payload.group_id !== 'g1') throw new Error('batch group payload mismatch');
  if (requests[0].payload.model_ids.join(',') !== 'm-new' || 'manual_price' in requests[0].payload) throw new Error('selected model payload mismatch');
  if (notices[0][0] !== 'success' || !notices[0][1].includes('新增 1 个，跳过 0 个，失败 0 个')) throw new Error('success summary mismatch');

  mode = 'duplicate';
  notices.length = 0;
  await context.actions.onAddAggregateMembers(controller);
  if (requests.length !== 2 || reloads !== 2) throw new Error('duplicate result must request and refresh once');
  if (notices[0][0] !== 'warning' || !notices[0][1].includes('新增 0 个，跳过 1 个，失败 0 个')) throw new Error('duplicate summary mismatch');

  mode = 'error';
  notices.length = 0;
  await context.actions.onAddAggregateMembers(controller);
  if (requests.length !== 3 || reloads !== 2) throw new Error('failed request must not refresh stale state');
  if (notices[0][0] !== 'error' || !notices[0][1].includes('添加成员失败')) throw new Error('error feedback mismatch');
  console.log('V063_COMBINED_ADD_UI_FLOW_OK');
})();
'''
    assert run_node(script) == "V063_COMBINED_ADD_UI_FLOW_OK"
