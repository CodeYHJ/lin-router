const DashboardTab = {
  ...
  focusDescriptor(element) {
    const descriptor = {
      ...
      copyTemplate: element.dataset?.copyTemplate || '',
      accessGroup: '',
      clientTemplateGroup: '',
    ...
    const clientTemplate = element.closest?.('[data-client-template-group]');
    descriptor.accessGroup = accessGroup?.dataset.accessGroup || '';
    descriptor.clientTemplateGroup = clientTemplate?.dataset.clientTemplateGroup || '';
    return descriptor;
  },
  ...
  restoreViewState(panel, viewState) {
    ...
    } else if (focus.copyTemplate) {
      target = Array.from(panel.querySelectorAll('[data-copy-template]')).find(item => item.dataset.copyTemplate === focus.copyTemplate);
    } else if (focus.clientTemplateGroup) {
      const template = Array.from(panel.querySelectorAll('[data-client-template-group]')).find(item => item.dataset.clientTemplateGroup === focus.clientTemplateGroup);
      target = template?.querySelector('summary') || null;
  ...
  flowContent(flow) {
    ...
    const content = {
      ...
      S3: {
        ...
        primary: `<button type="button" class="btn-primary" data-dashboard-action="copy-client" data-group-id="${groupId}">复制客户端配置</button>`,
      ...
      S4: {
        ...
        primary: '<button type="button" class="btn-primary" data-dashboard-action="new-aggregate">创建智能路由</button>',
        secondary: `<button type="button" class="btn-secondary" data-dashboard-action="copy-client" data-group-id="${groupId}">复制客户端配置</button>`,
      },
      E1: {
        title: '连接组需要处理',
  ...
  renderFlowCard(flow) {
    ...
    const item = flow.primary;
    const compact = ['S3', 'S4'].includes(flow.code);
    const summaryOpen = this._openFlowSummaries.has(flow.code);
    if (compact) {
      return `
        <section class="dashboard-flow-card dashboard-flow-compact status-${Utils.escapeHtml(item?.code || flow.code)}" data-dashboard-flow-card>
          <div class="dashboard-flow-summary-row">
            <div>
              <div class="dashboard-flow-status">当前摘要</div>
              <h3>${content.title}</h3>
              <p>${content.facts}</p>
            </div>
            ${content.primary}
          </div>
          <details class="dashboard-flow-details" data-dashboard-flow-summary="${flow.code}" ${summaryOpen ? 'open' : ''}>
            <summary>展开状态说明</summary>
            <div class="dashboard-flow-detail-content">
              ${this.renderFlowDetails(flow)}
              ${content.secondary ? `<div class="dashboard-flow-actions">${content.secondary}</div>` : ''}
            </div>
          </details>
        </section>`;
    }
      ...
      return `
        <details class="dashboard-access-card dashboard-access-group" data-dashboard-access-group="${Utils.escapeHtml(group.id)}" ${open ? 'open' : ''}>
          ...
          <div class="dashboard-access-content">
            ...
            <div class="copy-row"><span>API Key</span><code>${Utils.escapeHtml(group.route_key || '未生成')}</code><button type="button" class="btn-secondary btn-sm" data-copy-value="${Utils.escapeHtml(group.route_key || '')}" ${group.route_key ? '' : 'disabled'}>复制</button></div>
            ...
            ${this.renderClientTemplates(group, model, baseUrl)}
          </div>
        </details>`;
    }).join('')}</div>`;
  },
  ...
  attachEvents(panel) {
    ...
    panel.addEventListener('click', event => {
      const target = event.target.closest('[data-copy-value], [data-copy-client-group], [data-copy-template], [data-dashboard-action]');
      ...
      if (target.dataset.copyTemplate !== undefined) {
        Utils.copy(target.dataset.copyTemplate || '').then(ok => ok ? Toast.success('客户端模板已复制') : Toast.error('复制失败'));
        return;
      }
    ...
    panel.addEventListener('toggle', event => {
      ...
      if (details.dataset.clientTemplateGroup) {
        if (details.open) this._openClientTemplates.add(details.dataset.clientTemplateGroup);
        else this._openClientTemplates.delete(details.dataset.clientTemplateGroup);
      }
