...
def render_index_page(runtime_paths: Any | None = None, *, optional_runtime_script: str = "") -> str:
    paths = runtime_paths or ServerPlatform()
    page_path = paths.get_resource_path("index.html")
    if not page_path.exists():
        page_path = paths.get_resource_path("static", "index.html")
    if not page_path.exists():
        page_path = paths.get_resource_path("web", "shared", "index.html")
    html = page_path.read_text(encoding="utf-8")
    return html.replace("__AUTO_MODEL_NAME__", DEFAULT_AUTO_MODEL_NAME).replace(
        "__OPTIONAL_RUNTIME_SCRIPT_TAG__", optional_runtime_script
    )
...
class ArkProxyRouter:
    ...
    def __init__(
        ...
        self.platform = platform or ServerPlatform(getattr(store, "path", None))
    ...
    def _resolve_log_file(self) -> Path:
        candidates = [
            self.platform.get_log_dir() / "lin-router-logs.jsonl",
...
class RouterHandler(BaseHTTPRequestHandler):
    ...
    def _runtime_paths(self) -> Any:
        return getattr(self.server, "platform", None) or ServerPlatform()

    def _render_index_page(self) -> str:
        return render_index_page(
            getattr(self.server, "platform", None),
            optional_runtime_script=getattr(self.server, "optional_runtime_script", ""),
        )
...
def create_server(
    ...
    *,
    ...
    optional_resource_root: str | Path | None = None,
    ...
    runtime_platform = platform or ServerPlatform(config)
    return create_application_server(
        ...
        optional_resource_root=Path(optional_resource_root).resolve() if optional_resource_root else None,
