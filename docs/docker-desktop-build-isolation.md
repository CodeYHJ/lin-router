...
- [ ] Docker 最终镜像不含 Desktop package、依赖、资源和构建工具（复制清单和依赖契约已通过；等待 Docker Hub 发布后的本地 pull 验收）。
...
- 状态：本地实现完成，待目标 runner 确认 publish 顺序。
- 依赖：DP1。
...
- 状态：README 验收入口已完成，待发布后的真实本地 pull 结果。
...
- 当前环境没有 Docker，因此尚未执行 publish 后的真实 `docker pull`、容器 health、API、镜像内容和 named volume 持久化验收；这些仍是完成定义中的外部证据缺口。
