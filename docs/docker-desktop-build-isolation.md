...
| 当前路径 | 目标路径/处理方式 |
|---|---|
| `app.py` | 实现拆入 `linrouter_server/`，最终只保留兼容启动入口 |
| `desktop.py` | 实现迁入 `linrouter_desktop/tray.py`，最终只保留兼容启动入口 |
| `linrouter/__main__.py` | 保持 Desktop 兼容入口，转发到 `linrouter_desktop.__main__` |
| `linrouter_platform/` | 迁入 `linrouter_desktop/platform/`，完成后删除旧 package |
| `settings_store.py` | 公共存储/校验迁入 `linrouter_server/settings.py`；桌面 schema 迁入 `linrouter_desktop/settings.py` |
| `upstream_client.py` | 暂归 `linrouter_server`；不要在本次顺便重写 |
| `debug_capture.py` | 暂归 `linrouter_server`；不要在本次顺便重写 |
| `static/` | 公共资源迁入 `web/shared/`，桌面设置逻辑迁入 `web/desktop/` |
| `LinRouter.spec` | 迁入 `packaging/desktop/LinRouter.spec` |
| `scripts/build.sh` | 迁入 `packaging/desktop/build.sh` |
...
根目录兼容入口只保证现有启动命令和公共导入，不继续承载业务实现：
...
如果现有测试导入了 `app.ArkProxyRouter`、`app.RouterHandler`、常量或辅助函数，兼容入口可以暂时显式 re-export；不要使用通配符导出来掩盖边界。测试应逐步改为导入实际 owner package。
...
如果兼容期仍需根级 `upstream_client.py` 或 `debug_capture.py`，可以临时白名单复制，并创建后续迁移事项；不得因此改回 `COPY . .`。
...
- 迁移 `app.py` 实现到明确 owner；根入口仅兼容转发。
- 更新测试导入路径。
...
- 可用时并行运行二者。
- 验证双方只写各自目录。
- 删除旧 package、旧构建入口和过期文档引用；保留根 `requirements.txt` 兼容入口。
...
- [ ] `requirements.txt` 和角色化 `requirements/` 清单已提交。
- [ ] server 与 Desktop 使用不同环境目录或等价的依赖隔离方式。
- [ ] 目标 package 和 packaging 目录已建立，旧实现文件只剩兼容入口或已删除。
- [ ] `linrouter_server` 和 `linrouter_core` 不导入 Desktop。
- [ ] Docker API 不暴露 Desktop 设置，错误不会导致连接断开。
- [ ] Docker 静态资源不含 Desktop 设置代码。
- [ ] Docker 最终镜像不含 Desktop package、依赖、资源和构建工具。
- [ ] Desktop 产物不含 Docker 专属文件或依赖。
- [ ] Desktop 当前功能和发布产物保持可用。
- [ ] Docker/Desktop 工作目录、缓存、产物目录完全分开。
- [ ] 两种构建顺序和可行时的并行构建均通过。
- [ ] 全量测试、静态边界测试、真实 server smoke、镜像 smoke 和平台打包均通过。
- [ ] README、CI、release workflow 与最终目录和依赖清单一致。
- [ ] `git diff --check` 通过，工作树没有非预期构建写入。
...
- `python desktop.py --help`、源码 Desktop 启动和 PyInstaller 冻结入口必须指向同一套组装逻辑。
...
- B3 已处理一部分：spec 改用根级 `desktop.py` 兼容入口，避免直接把 `linrouter_desktop/platform` 放到顶层脚本路径；Windows/macOS runner 仍需真实验证。
...
本节保留为初始拆分记录；其最新状态以 18.8、18.10 和 18.11 为准。
...
- Server 环境全量测试：`264 passed`。
- 隔离/备份/Desktop composition/依赖、workflow 及入口隔离专项测试：`23 passed`。
...
- 优先级：P1。
- 依赖：U1、U3。
- 修改范围：`README.md`、`start-preview-18409.bat`。
- 实施要求：
...
- 本轮结果：全量测试 `264 passed`；隔离、依赖、workflow 与入口契约专项测试 `23 passed`；两份 workflow YAML 解析、compileall、JavaScript `node --check`、Shell `bash -n`、Server/Core 依赖和标识扫描、Docker/Desktop manifest 扫描、Server CLI smoke、`git diff --check` 均通过。当前 uv 环境未提供 `pip` 模块，使用当前环境的 `uv pip check` 验证 Server/Desktop 依赖均无冲突；这只是本地工具验证记录，不改变项目依赖管理器无关原则。
