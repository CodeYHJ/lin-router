...
全部满足才可以宣称完成。下列状态以第 18.16 节的最新复检为准：
...
- [ ] Docker 最终镜像不含 Desktop package、依赖、资源和构建工具（复制清单和依赖契约已通过；等待 main 发布后的本地 pull 验收）。
...
- [ ] 两种构建方向的写集保护均通过（Desktop→Docker 已取得 Windows/macOS runner 证据；Docker→Desktop 需在当前唯一 Docker workflow 重跑确认）。
- [ ] 全量测试、静态边界测试、平台打包和本地 Docker pull/server smoke 均通过（代码侧已完成，目标发布与本地运行证据待补）。
...
- 自动发布只允许由 `main` 分支相关 Server/Docker 文件更新触发；不保留定时任务，也不允许功能分支覆盖 `latest`。
- PR 继续执行 Dockerfile build、image inspect 和 Docker→Desktop 写集保护，但不得登录或推送 Docker Hub。
...
#### DP1：收口为唯一 Docker workflow，并限制 main 发布
...
- 实施要求：
  ...
  - 保留 `workflow_dispatch`，但 publish job 同时限制 `github.ref == 'refs/heads/main'`；从其他 ref 手动触发时只能验证，不能发布 `latest`。
  - 保留现有 Docker/Server 路径过滤，不因文档或 Desktop-only 变化发布镜像。
  - 仓库中不得再存在第二个 Docker Hub publish workflow。
- 验收标准：PR 不读取 Docker Hub secrets；非 main push 不触发自动发布；main 的相关变更会进入 build 和 publish 流程。

#### DP2：建立 build 通过后才能 publish 的依赖 Gate

- 状态：本地实现完成，待 main runner 确认 publish 顺序。
- 依赖：DP1。
- 修改范围：`.github/workflows/docker-build.yml`、`tests/test_dependency_isolation_contract.py`。
- 实施要求：
...
- 状态：本地实现完成，待 CI workflow 重跑确认。
- 修改范围：`.github/workflows/ci.yml`、`tests/test_build_isolation_contract.py`、本文相关状态说明。
- 实施要求：
  - 完整删除 `build_docker_smoke` job，不保留 `if: ${{ false }}` 死代码。
  - 删除“smoke 必须保持暂停”的旧契约测试，改为确认 `ci.yml` 不包含 Docker build/run job，Docker 构建和发布只归属 `docker-build.yml`。
...
- 状态：README 验收入口已完成，待 main 发布后的真实本地 pull 结果。
- 依赖：DP1、DP2 以及一次成功的 main publish。
- 修改范围：`README.md`、本文；如需要可增加只读/可清理的本地验收脚本，但不得重新放回 GitHub CI smoke。
- 验收范围：
...
- 验收标准：README 给出可复制执行的 pull、启动、检查和清理命令；真实本地运行结果由用户确认后再记为通过。
...
- publish 仅允许 `main`，并显式依赖 build；PR 和非 main 手动触发不读取 Docker Hub secrets、不发布 `latest`。
- 全量测试 `277 passed`；当时三份 workflow YAML 解析、Shell 语法和 `git diff --check` 通过。移除 `package.yml` 后应以 18.17 的最新回归结果为准。
- 当前环境没有 Docker，因此尚未执行 main publish 后的真实 `docker pull`、容器 health、API、镜像内容和 named volume 持久化验收；这些仍是完成定义中的外部证据缺口。
...
- 当前 workflow 只剩 `ci.yml` 与 `docker-build.yml`：前者负责源码测试和 Desktop preview，后者负责 Docker build、边界校验和 main 分支 Docker Hub publish。
...
- 当前没有已知本地代码不确定性；剩余证据仅为 main 分支 Docker Hub publish 真实执行，以及发布后由用户本地 `docker pull` 完成容器运行验收。
