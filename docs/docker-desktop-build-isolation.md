...
全部满足才可以宣称完成。下列状态以第 18.19 节的最新复检为准：
...
- [x] Docker 最终镜像不含 Desktop package、依赖、资源和构建工具（复制清单、依赖边界和容器内运行时验收均已通过）。
- [x] Desktop 产物清单不收集 Docker 专属文件或依赖。
- [ ] Desktop 当前功能和发布产物保持可用（Windows/macOS 构建已通过，macOS DMG 已安装并确认托盘和启动正常；Windows 运行时仍需单独确认）。
- [x] Docker/Desktop 工作目录、缓存、产物目录完全分开。
- [x] 两种构建方向的写集保护均通过（Desktop→Docker 已取得 Windows/macOS runner 证据，Docker→Desktop 已随成功的 Docker workflow 完成）。
- [x] 全量测试、静态边界测试、平台打包和本地 Docker pull/server smoke 均通过。
- [x] README、CI 和唯一 Docker workflow 与最终目录和依赖清单一致；自动 release workflow 已按当前决定移除。
...
- 本轮结果：`ci.yml`、`package.yml` 已删除 `setup-uv`、`UV_PROJECT_ENVIRONMENT`、`uv sync`、`uv run`，改用 `actions/setup-python` 的 pip cache、`python -m pip`、`requirements/test.txt` 和 `requirements/package.txt`；Docker smoke 多行 Python 当时改为 YAML block 内的 heredoc，后续由 18.16 的 DP3 整体删除。Node 20 action 已升级为 Node 24 对应版本：`checkout@v6`、`setup-python@v6`、`upload-artifact@v7`、`download-artifact@v8`、`action-gh-release@v3`。Release workflow 的停用 `Verify tagged source` job 后续按“不隐藏未使用代码”的规则直接删除，`build` 只依赖 `resolve`；`resolve` 会先检查 tag 是否包含当前 requirements、packaging 和 package 布局，避免 Windows/macOS 在缓存阶段分别失败。独立 `docker-build.yml` 后续按 18.16 收口为 build、边界验证和 Docker Hub publish 的唯一 Docker workflow。三份 workflow 已通过本地 YAML 解析和契约测试。
...
本节是 2026-07-18 的历史状态快照；Docker Hub 发布和本地 pull 验收的当前决策及状态由 18.16 覆盖。
...
- 当时 `ci.yml` 的 `build_docker_smoke` job 使用显式 `if: ${{ false }}` 暂停；该历史 job 已由 18.16 的 DP3 删除。独立 `docker-build.yml` 后续负责 build、inspect 和 Docker Hub publish，本地运行验收按 DP4 执行。
...
本节的 Docker workflow 与 runtime smoke 描述属于迁移过程记录；当前 Docker Hub 发布和本地 pull 验收以 18.16 为准。
...
- 历史状态：仍待 Windows/macOS 使用新 PyInstaller 入口重新构建，以及修改后的 Docker/Desktop 写集 workflow 在目标 runner 上通过；当前状态以 18.16 和用户后续 runner 结果为准。
...
本节保留迁移复检时的历史证据；Windows/macOS build、Docker workflow 和运行验收的最新状态由 18.16 及用户后续确认覆盖。
...
- `v0.6.5` 的 macOS build 已成功；Windows build 曾在边界校验步骤失败。
...
- `ci.yml` 和当时的 `package.yml` 中所有 `Verify Desktop build preserved Docker outputs` 步骤统一显式使用了 `shell: bash`；macOS 行为保持不变，Windows 与快照步骤使用相同的环境变量语义。`package.yml` 后续按 18.17 直接移除。
- 构建隔离契约测试会检查 CI 中两个校验步骤和 Release 中一个校验步骤均固定为 Bash，防止再次回退到平台默认 shell。
- 修复提交为 `093bff9`，随后 `v0.6.6` 的 Windows build 已由用户确认通过；本节的失败记录保留作根因追踪。

### 18.16 Docker Hub 发布流程收口任务（执行记录）

本节记录用户对 Docker 后续流程的最新决定，覆盖 18.12、18.14 中“未来在 `ci.yml` 启用 Docker runtime smoke”的旧安排。当前工作树已经包含把 Docker Hub publish 合并进 `docker-build.yml`、删除重复 publish workflow 的实现；执行者必须基于现有工作树继续验证，不得丢弃这些未提交改动。

#### 已确认决策

- 仓库只保留一个独立 Docker workflow：`.github/workflows/docker-build.yml`。
- Docker Hub 使用既有 `DOCKER_USERNAME`、`DOCKER_TOKEN` secrets，目标镜像固定为本项目的 `codeyhj/agent-router:latest`。
- 自动 push 仍只由 `main` 分支相关 Server/Docker 文件更新触发；不保留定时任务。
- publish job 不增加事件或分支判断，只要本次 workflow 被触发且 build 成功就推送 `latest`；因此从功能分支手动运行或由 PR 触发时也会发布。
- 不再保留 `ci.yml` 中被 `if: ${{ false }}` 禁用的 Docker runtime smoke。实际运行验收改为：CI 发布镜像后，本地 `docker pull`，再验证容器启动和应用功能。
- 未启用的 workflow、job 和步骤不得用整段注释或恒假条件隐藏；不再使用的代码直接删除，原因和恢复思路只记录在本文档。
- Linux Desktop 仍不支持；本节不改变 Server/Desktop 的目录、依赖和打包边界。

#### DP1：收口为唯一 Docker workflow

- 状态：实现和目标 runner 验证完成。
...
- 实施要求：
  ...
  - 保留现有 Docker/Server 路径过滤，不因文档或 Desktop-only 变化发布镜像。
  - 仓库中不得再存在第二个 Docker Hub publish workflow。
- 验收标准：非 main push 不触发自动运行；main 的相关变更会自动进入 build 和 publish 流程；任意 ref 的手动运行和 PR 触发在 build 成功后也会进入 publish。

#### DP2：建立 build 通过后才能 publish 的依赖 Gate

- 状态：实现和目标 runner 验证完成；镜像已成功发布并由本地 Compose 拉取运行。
- 依赖：DP1。
- 修改范围：`.github/workflows/docker-build.yml`。
- 实施要求：
  - publish job 显式 `needs: build`。
...
- 状态：实现和 CI 验证完成。
- 修改范围：`.github/workflows/ci.yml`、本文相关状态说明。
- 实施要求：
  - 完整删除 `build_docker_smoke` job，不保留 `if: ${{ false }}` 死代码。
  - 删除“smoke 必须保持暂停”的旧契约测试；Docker 构建和发布只归属 `docker-build.yml`。
  - 不删除 Dockerfile、entrypoint、healthcheck 或 Server-only 镜像边界测试。
- 验收标准：GitHub Actions 只显示一个独立 Docker workflow；普通 CI 只负责源码、测试和 Desktop preview，不构建或运行 Docker。

#### DP4：补充本地 pull 与运行验收入口

- 状态：真实本地 pull、Compose 启动和完整运行验收已完成。
- 依赖：DP1、DP2 以及一次成功的 publish。
- 修改范围：`README.md`、本文；README 只保留日常 Compose 启停入口，一次性调试脚本不作为交付文件保留。
- 验收范围：
  - `docker pull codeyhj/agent-router:latest` 成功。
  - 使用端口映射和 named volume 启动，容器达到 `healthy`。
  - `/health` 和最小应用/API 可访问。
  - 镜像中不存在 `linrouter_desktop`、Desktop packaging、pystray、Pillow 和 PyInstaller。
  - 容器使用非 root UID，配置写入 `/data`，重建容器后 named volume 数据仍存在。
  - 验收结束后明确清理测试容器；named volume 是否保留由执行者在命令中明确说明，不得隐式删除用户数据。
- 验收标准：README 给出日常 pull、启动和停止命令；真实本地运行结果由用户确认后记为通过。

#### DP5：最终回归与状态同步

- 状态：Docker 范围的本地回归、目标环境证据和状态同步均完成。
...
- DP1–DP4 的代码和文档实现已完成；DP5 的本地回归、状态同步和未使用 workflow 代码清理已完成。
- 仓库只保留 `ci.yml` 和 `docker-build.yml` 两份 workflow；`ci.yml` 不再包含 Docker build/run job，`package.yml` 和独立 Docker Hub publish workflow 均已直接移除。
- publish 不设置事件或分支判断，仅显式依赖 build；手动触发、PR 或 main 自动触发都在 build 成功后发布 `latest`。
- 清理 GitHub Actions、README 和 Compose 文本测试后的完整业务与代码测试为 `267 passed`，`git diff --check` 通过。
- Docker Hub publish、本地 pull、Compose 启动、Web/health、非 root、镜像内容、`/data` 写入和 named volume 跨容器重建持久化均已取得真实运行证据。
...
### 18.17 Release workflow 移除（2026-07-19）
...
- 最新本地回归状态由 18.18、18.19 覆盖：完整业务与代码测试 `267 passed`，`git diff --check` 通过。
- 当前没有已知 Docker 代码不确定性；Docker Hub publish 和本地完整运行验收均已完成。
...
### 18.19 Docker 真实运行验收（2026-07-20）

- 用户已确认 CI 发布镜像可通过 Compose 启动，并可经映射端口打开 Web 页面。
- 容器内真实检查确认 UID `10001`、`/data` 可写、`/health` 可访问、Server/Core 模块存在，以及 Desktop package、路径、pystray、Pillow 和 PyInstaller 不存在。
- 已在旧容器写入 `/data` 临时标记，使用同一 named volume 强制重建容器，并在新容器中成功读取和清理该标记；volume 跨容器重建持久化通过。
- 验收使用的一次性调试脚本不作为项目交付文件保留，README 也不保留该调试流程。
- 删除了 README 与 Compose 的纯文本 pytest；它们不属于业务行为测试。当前完整回归为 `267 passed`。

### 18.20 代码侧收口任务（待分配，2026-07-20）

本节只定义后续任务，本轮不执行实现、测试、构建、提交或 tag 操作。当前 Docker/Desktop 目录、依赖、构建和运行边界已经完成验收；以下任务用于清理代码侧残留并进一步校准测试职责，不得把范围扩展为大规模架构重构。

#### CODE-1：移除 Desktop Server 创建的旧签名回退

- 状态：已完成（2026-07-20）。
- 优先级：P1。
- 问题归类：Docker/Desktop 迁移引入的兼容残留。
- 主要范围：`linrouter_desktop/tray.py`、`tests/test_desktop_launch_contract.py`。
- 当前问题：Desktop 启动先用完整依赖参数调用 `create_server`；一旦捕获消息含 `unexpected keyword argument` 的 `TypeError`，便使用旧的三参数签名重试。该分支目前主要是在兼容测试中的旧 fake，而不是当前生产接口。
- 风险：回退调用会丢失 Desktop settings store、平台实现、可选能力和资源/脚本路径等注入；若 `create_server` 内部恰好抛出相似文本的 `TypeError`，还可能被误判为签名不兼容并重复启动。
- 实施要求：
  1. 删除基于 `TypeError` 文本匹配的旧三参数回退，Desktop 每次启动只允许按当前正式签名调用一次 `create_server`。
  2. 更新测试 fake，使其接受并校验当前参数；不得为保留旧 fake 而在生产代码继续兼容旧签名。
  3. 回归测试至少覆盖平台实现、可选能力、共享 settings store，以及存在时的资源根、URL 前缀和辅助脚本等注入。
  4. 新增失败行为测试：`create_server` 抛出 `TypeError` 时启动失败、只调用一次，且不得留下半启动的 Server 或后台线程。
- 验收标准：Desktop 正常启动路径保持完整注入；错误路径不重试旧签名；相关 Desktop 启动和 composition 行为测试通过。
- 实际结果：`LinRouterTray.start_server()` 已删除旧三参数回退，只按正式完整签名调用一次；新增测试确认平台、可选能力、共享 settings store、资源根、URL 前缀和辅助脚本均被注入，并确认 `TypeError` 时只调用一次且不残留 Server/线程。Desktop 启动专项 `6 passed`，与隔离行为组合专项 `20 passed`。
- 禁止扩展：本任务不处理 H1–H4，不重构 `application.py`，不改变 Server 对外接口。

#### TEST-1：从 pytest 中移除非业务基础设施文本契约

- 状态：已完成（2026-07-20）。
- 优先级：P1。
- 问题归类：测试职责收口，不是生产功能缺陷。
- 主要范围：
  - `tests/test_dependency_isolation_contract.py`
  - `tests/test_build_isolation_contract.py`
  - `tests/test_repository_layout_contract.py`
  - `tests/test_v063_release_metadata.py`
  - `tests/test_dashboard_runtime_ui.py`
  - 其他读取 README、Dockerfile、Compose、requirements、spec、脚本或仓库布局并只匹配文本的测试
- 实施要求：
  1. 删除仅断言 requirements 分层、Dockerfile 文本、uv 文件不存在、README 命令或 BAT 路径的测试；若文件不再包含业务行为测试，直接删除整个测试文件。
  2. 拆分 `test_build_isolation_contract.py` 中混合的职责：保留 Desktop/Server 共享 settings、schema 隔离、备份过滤和 round-trip 等业务行为；移除源码 token 扫描、Docker context、PyInstaller spec、解释器选择、写集快照和根目录布局等基础设施文本断言。
  3. 删除仓库文档布局、历史 release 元数据和 README 内容的 pytest；不得仅换目录或换文件名后继续保留同类字符串断言。
  4. `test_dashboard_runtime_ui.py` 中真实的 UI 业务行为可以保留，但应解除对 README 文案的耦合，并按实际业务行为命名。
  5. 全面盘点 `tests/` 内的文件读取测试：只有确实表达业务行为、且当前没有更合适运行 harness 的源码级 UI 行为检查可以保留；每项保留都应能说明它保护的用户可见行为。
  6. GitHub Actions 由真实 runner 验收，Docker 由真实 build/push/Compose 验收，Desktop 由 Windows/macOS 构建及运行验收；不得在 pytest 中重新模拟这些流程。
- 验收标准：
  - `tests/` 不再断言 workflow、README、Dockerfile、Compose、release 元数据或仓库目录必须包含某段文本。
  - Server/Desktop settings、API、备份、路由和 UI 业务行为覆盖不被削弱。
  - 清理后的完整业务测试通过，并在本文记录新的测试数量和删除/保留分类。
- 依赖：可与 CODE-1 独立实施；两项合并后必须再做一次完整回归。
- 实际结果：已删除依赖分层、Docker/Compose、README、仓库布局、release 元数据、Desktop build/spec、源码 import/token、旧 main 快照、facade 委托和 CI 顺序等纯文本/静态基础设施契约；保留 settings/schema、backup、composition、路由、流式传输、签名 helper 和真实 UI 行为测试。测试目录已无 README、Dockerfile、Compose、requirements、仓库布局或 release 元数据匹配。

#### MAC-1：确认并验收 macOS LaunchAgent 语义

- 状态：待决策、待 macOS 目标环境处理。
- 优先级：P2，不阻塞 Docker/Desktop 迁移完成。
- 问题归类：原 main 已存在的平台历史问题，不是本次迁移导致的缺陷。
- 主要范围：`linrouter_desktop/platform/darwin.py` 及对应平台测试。
- 当前问题：LaunchAgent plist 使用 `KeepAlive: true`，可能在用户主动退出程序后立即重新拉起，这与通常的“仅登录时自动启动”语义不一致；现有状态检查主要验证 plist 内容，未完整验证 launchd 中的真实加载状态。
- 前置决策：明确产品语义是“登录时启动一次”还是“始终保持驻留”。建议采用前者，即保留登录启动能力但不强制 `KeepAlive`；执行者不得在未确认语义时自行改变用户退出行为。
- 实施与验收要求：
  1. 在真实 macOS 上验证启用、重复启用、禁用和重复禁用均可安全执行。
  2. 验证 plist 路径、program arguments、`launchctl` 加载/卸载状态与应用展示状态一致。
  3. 若采用“登录时启动一次”，验证用户主动退出后不会被立即拉起，而重新登录后会正常启动。
  4. 禁用后不得残留已加载服务或失效 plist；失败时应提供可诊断错误，不得只依赖文件是否存在。
  5. 增补能在非 macOS 环境运行的参数构造和错误分支单元测试，但最终结论必须以真实 macOS 验收为准。
- 禁止扩展：不新增 Linux Desktop 支持，不把 macOS launchd 行为纳入 Docker 验收，也不把真实平台能力伪装成纯文本测试。

#### FINAL-1：代码侧最终复检与文档同步

- 状态：已完成（2026-07-20）；MAC-1 仍为独立平台待决策项。
- 优先级：P1。
- 依赖：CODE-1、TEST-1。
- 验收要求：
  1. 复检 `linrouter_core`、`linrouter_server` 不导入 Desktop，Docker context 和镜像不包含 Desktop 代码或依赖；该项采用代码审查、实际构建和镜像检查，不新增 pytest 文本契约。
  2. 复检 Desktop 仍只按正式接口组合 Server/Core，且 macOS/Windows 的入口和打包目录未回退到根级兼容文件。
  3. 运行完整业务测试、必要的 Python/JavaScript/Shell 语法检查和 `git diff --check`；Windows/macOS/Docker 的流程状态只引用真实目标环境结果。
  4. 更新本文任务状态，明确剩余项属于迁移缺陷、原 main 历史问题还是长期架构债务，不得混写为同一阻塞项。
- 完成定义：CODE-1 不再存在旧签名兼容分支，pytest 职责按 TEST-1 收口，且没有发现新的 Docker/Desktop 迁移残留。
- 实际结果：CODE-1、TEST-1 已完成；清理后的全量业务测试 `235 passed`，Python compileall、相关 JavaScript/Shell 检查和 `git diff --check` 通过。Docker/Desktop 构建与运行证据继续沿用 18.17–18.19 的真实 runner/容器验收；未发现新的迁移阻塞项。

#### 明确排除的工作

- H1–H4 的 settings 持久化、平台副作用事务和 backup partial commit 等原 main 历史问题。
- 拆分体量较大的 `linrouter_server/application.py` 或 `linrouter_core/runtime/router_runtime.py`。
- 补全当前未进入正式生产打包链路的 `frontend/` 实验目录。
- Linux Desktop、GitHub Actions pytest、一次性 Docker 调试脚本，以及本地 `.venvs`、缓存和临时文件清理。
- 提交、推送、创建 tag 或发布；这些操作必须由用户另行授权。
...
MAC-1（独立平台历史任务，需要产品语义决策和真实 macOS）
```

#### 18.20 执行后的剩余项

- `MAC-1` 是原 main 的 macOS LaunchAgent 历史问题，不属于本次 Docker/Desktop 迁移缺陷；需要先决定“登录启动一次”或“始终驻留”，再在真实 macOS 上验收。
- H1–H4、Server/Core 大文件拆分、frontend 实验目录和 Linux Desktop 仍按“明确排除的工作”保留，均不阻塞当前迁移边界。
