# Lin Router v0.5.1 Release Checklist
...
- [ ] 生成 `dist/LinRouter-v0.5.1-win-x64.zip`。
- [ ] 生成 `dist/LinRouter-Setup-v0.5.1-win-x64.exe`。
- [ ] 安装包 smoke test：`LinRouter-Setup-v0.5.1-win-x64.exe --silent --no-run --dir <temp-dir>`。
- [ ] 安装后存在 `<temp-dir>/dist/LinRouter.exe` 和 `<temp-dir>/uninstall.cmd`。
- [ ] 运行脱敏扫描：`python scripts/release_guard.py dist/LinRouter-v0.5.1-win-x64.zip dist/LinRouter-Setup-v0.5.1-win-x64.exe`。
