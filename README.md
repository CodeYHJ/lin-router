...
dist\LinRouter-Setup-v0.5.5-win-x64.exe
```

推荐分发安装包 `LinRouter-Setup-v0.5.5-win-x64.exe`；单文件免安装使用时可直接运行 `LinRouter_windows.exe`。
...
# -> dist/LinRouter_windows.exe + dist/LinRouter-v0.5.5-win-x64.zip + dist/LinRouter-Setup-v0.5.5-win-x64.exe

# 指定安装包版本号
scripts/build.sh --target win32 --installer --version 0.5.5
# -> dist/LinRouter-Setup-v0.5.5-win-x64.exe
...
v0.5.5 发布前检查清单见 `scripts/release-checklist-v0.5.5.md`；构建脚本会自动对 zip / setup 产物执行脱敏扫描。
...
1. 下载 `LinRouter-Setup-v0.5.5-win-x64.exe`，双击安装；如果 Windows Defender 提示未知发布者，确认来源是本项目发布包后选择“仍要运行”。
